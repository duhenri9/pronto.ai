...
const BLOCKED_PATTERNS: RegExp[] = [
  ...
  /ignor(e|ar)\s+(todas\s+as\s+|as\s+|os\s+|todas\s+)?(instruç|instruc|orientaç|comando)/i,
  /finja\s+que\s+(voc[êe]|és)/i,
  /a\s+partir\s+de\s+agora\s+(voc[êe]|és)/i,
  /revele\s+(seu|o)\s+(sistema|prompt|funcionamento)/i,
  /esqueça\s+(tudo|todas\s+as\s+instruç|as\s+instruç|as\s+orientaç|o\s+que\s+foi\s+dito)/i,
